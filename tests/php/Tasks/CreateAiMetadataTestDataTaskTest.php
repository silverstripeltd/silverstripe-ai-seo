...
use SilverstripeLtd\AiMetadata\Tasks\CreateAiMetadataTestDataTask;
 ...
 */
class CreateAiMetadataTestDataTaskTest extends SapphireTest
{
    private const SCI_FI_TITLE = 'AI Metadata Test - Sci-Fi Spaceport Briefing';
    private const FANTASY_TITLE = 'AI Metadata Test - Fantasy Guild Charter';
    private const COUNCIL_BLOCKS_TITLE = 'AI Metadata Blocks - Local Council Services';
    private const FINANCE_BLOCKS_TITLE = 'AI Metadata Blocks - Financial Services Updates';
    ...
    public function testTaskCreatesFixturePagesAndRemovesAllDemoPages(): void
    {
        ...
        $legacyPage->write();
        $metadata = $legacyPage->getOrCreateAiMetadata();
        ...
        $task = new CreateAiMetadataTestDataTask();
        $task->run(new ArrayInput([]), new PolyOutput(PolyOutput::FORMAT_ANSI));

        $this->assertNull(GeneratedMetadata::get()->filter('ParentID', $legacyPage->ID)->first());
        ...
        $sciFi = AiPage::get()->filter('Title', CreateAiMetadataTestDataTaskTest::SCI_FI_TITLE)->first();
        ...
        $sciFiLive = Versioned::get_by_stage(AiPage::class, Versioned::LIVE)
            ->filter('Title', CreateAiMetadataTestDataTaskTest::SCI_FI_TITLE)
            ->first();
        $this->assertNotNull($sciFiLive);

        $fantasy = AiPage::get()->filter('Title', CreateAiMetadataTestDataTaskTest::FANTASY_TITLE)->first();
        $this->assertNotNull($fantasy);
        $fantasyLive = Versioned::get_by_stage(AiPage::class, Versioned::LIVE)
            ->filter('Title', CreateAiMetadataTestDataTaskTest::FANTASY_TITLE)
            ->first();
        $this->assertNotNull($fantasyLive);

        $council = AiBlocksPage::get()
            ->filter('Title', CreateAiMetadataTestDataTaskTest::COUNCIL_BLOCKS_TITLE)->first();
        ...
        $councilLive = Versioned::get_by_stage(AiBlocksPage::class, Versioned::LIVE)
            ->filter('Title', CreateAiMetadataTestDataTaskTest::COUNCIL_BLOCKS_TITLE)
            ->first();
        $this->assertNotNull($councilLive);

        $finance = AiBlocksPage::get()
            ->filter('Title', CreateAiMetadataTestDataTaskTest::FINANCE_BLOCKS_TITLE)->first();
        ...
        $financeLive = Versioned::get_by_stage(AiBlocksPage::class, Versioned::LIVE)
            ->filter('Title', CreateAiMetadataTestDataTaskTest::FINANCE_BLOCKS_TITLE)
            ->first();
