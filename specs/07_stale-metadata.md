...
1. **At generation time:** After extracting page content (via the content extraction pipeline in `specs/03_content-extraction.md`), compute an MD5 hash of the extracted text. Store this hash as `ContentHash` on the `GeneratedMetadata` record alongside the generated metadata.
...
- `ContentHash` field (Varchar) on the `GeneratedMetadata` DataObject
