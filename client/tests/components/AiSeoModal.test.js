...
const mockCreateDefaultFixture = () => ({
  ...
  reviewLabel: 'I have reviewed the generated AI SEO',
...
const resetSubmitHandlers = () => {
  mockSubmitHandlers = {
    doRegenerate: jest.fn().mockResolvedValue({ meta: { hadGeneratedSeo: false } }),
...
jest.mock('lib/Config', () => ({
  ...
  default: {
    getSection: () => ({
      form: {
        aiSeoForm: {
          schemaUrl: '/admin/ai-seo/schema/AiSeoForm',
        },
      },
    }),
  },
...
jest.mock('components/FormBuilderModal/FormBuilderModal', () => {
  ...
  return ({
    ...
    const regenerateLabel = fields.GeneratedAt ? 'Regenerate' : 'Generate SEO';
    ...
    return ReactModule.createElement(
      ...
      ReactModule.createElement(
        'div',
        { 'data-field-path': 'AiSeoKeyTopicsHeader' },
      ...
      ReactModule.createElement(
        ...
        ReactModule.createElement('span', {
          'data-ai-seo-count': 'MetaDescription',
          'data-ai-seo-max': String(fixture.metaDescriptionMax),
        }),
      ),
      ReactModule.createElement(
        'div',
      ...
      ReactModule.createElement(
        ...
        ReactModule.createElement('span', {
          'data-ai-seo-timestamp': fixture.timestamp,
        }, fixture.timestamp),
      ),
      ReactModule.createElement(
        'div',
...
} from '../../src/components/AiSeoModal';
...
test('buildSchemaUrl encodes fqcn and appends record id', () => {
  ...
  expect(url).toBe('/admin/ai-seo/schema/AiSeoForm/42?fqcn=My%5CClass');
});

test('buildModalSelector handles custom and default classes', () => {
  expect(buildModalSelector('ai-seo-modal extra')).toBe('.ai-seo-modal.extra');
  expect(buildModalSelector('')).toBe('.ai-seo-modal');
  expect(buildModalSelector(null)).toBe('.ai-seo-modal');
});
...
test('syncInlineRegenerateAction inserts a proxy action above the key topics header', () => {
  ...
  status.setAttribute('data-field-path', 'AiSeoStatus');
  const keyTopicsHeader = document.createElement('div');
  keyTopicsHeader.setAttribute('data-field-path', 'AiSeoKeyTopicsHeader');
  ...
  sourceButton.innerHTML = '<span class="btn__title">Generate SEO</span>';
  ...
  const inlineAction = modal.querySelector('.ai-seo-modal__inline-regenerate-action');
  ...
  expect(proxyButton?.textContent).toContain('Generate SEO');
  expect(proxyButton?.className).toContain('btn btn-primary');
  expect(sourceButton.getAttribute('aria-hidden')).toBe('true');
  expect(sourceButton.className).toContain('ai-seo-modal__source-action--hidden');
});
...
test('requires review confirmation before enabling submit for unreviewed metadata', async () => {
  ...
  expect(submitButton.getAttribute('title')).toBe('Please confirm you have reviewed the SEO before submitting.');
...
test('renders the regenerate proxy between status and the key topics header', async () => {
  ...
  const keyTopicsHeaderField = document.querySelector('[data-field-path="AiSeoKeyTopicsHeader"]');

  await waitFor(() => {
    const inlineAction = document.querySelector('.ai-seo-modal__inline-regenerate-action');
    expect(inlineAction).not.toBeNull();
...
test('hides the apply action until AI SEO has been generated', () => {
  render(
    <AiSeoModalComponent
      fqcn="App\\Page"
      recordId={7}
  ...
  expect(screen.getByRole('button', { name: 'Generate SEO' })).not.toBeNull();
  expect(screen.queryByRole('button', { name: 'Apply SEO' })).toBeNull();
});
...
test('clicking the regenerate proxy triggers the real footer action', async () => {
  ...
  await waitFor(() => {
    expect(document.querySelector('.ai-seo-modal__inline-regenerate-action button')).not.toBeNull();
  });

  await act(async () => {
    fireEvent.click(document.querySelector('.ai-seo-modal__inline-regenerate-action button'));
  });

  expect(mockSubmitHandlers.doRegenerate).toHaveBeenCalledTimes(1);
  await waitFor(() => {
    expect(actions.toasts.info).toHaveBeenCalledWith('Generated AI SEO created. Review and apply to save.');
  });
});
...
test('shows regenerate and save toasts from FormBuilderModal submit callbacks', async () => {
  ...
  await act(async () => {
    fireEvent.click(screen.getByRole('button', { name: 'Apply SEO' }));
  });

  await waitFor(() => {
    expect(actions.toasts.success).toHaveBeenCalledWith('Generated AI SEO saved.');
  });
});
