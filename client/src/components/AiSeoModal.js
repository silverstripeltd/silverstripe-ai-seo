...
export const buildSchemaUrl = (fqcn, recordId) => {
  const { schemaUrl } = Config.getSection('SilverstripeLtd\\AiSeo\\Controllers\\AiSeoController').form.aiSeoForm;
...
const getModalConfig = () => {
  const controllerConfig = Config.getSection('SilverstripeLtd\\AiSeo\\Controllers\\AiSeoController') || {};
  return controllerConfig.form?.aiSeoForm || {};
};
...
const savedNoticeText = 'Generated AI SEO saved.';
const regenerateErrorText = 'Failed to refresh generated AI SEO.';
const saveErrorText = 'Failed to save generated AI SEO.';
...
/**
 * Determine whether generated SEO still needs review.
 *
 * @param {HTMLElement|null} modal
 * @returns {boolean}
 */
...
export const syncInlineRegenerateAction = (modal) => {
  ...
  const sourceButton = modal.querySelector(regenerateActionSelector);
  const keyTopicsHeaderField = findFieldElement(modal, 'AiSeoKeyTopicsHeader');
  const keyTopicsField = findFieldElement(modal, 'KeyTopicsDisplay');
  const statusField = findFieldElement(modal, 'AiSeoStatus');
...
/**
 * AI SEO modal wrapper around FormBuilderModal.
 *
 * @param {Object} props
...
const AiSeoModal = ({
  ...
  const className = modalConfig.className || 'ai-seo-modal';
  ...
  const updateLengthIndicators = useCallback(() => {
    ...
    const hints = modal.querySelectorAll('[data-ai-seo-count]');
    hints.forEach((element) => {
      const fieldName = element.getAttribute('data-ai-seo-count');
      const maxValue = parseInt(element.getAttribute('data-ai-seo-max') || '0', 10);
      const input = modal.querySelector(`[name="${fieldName}"]`);
      if (!input) {
        return;
      }
  ...
  const updateTimestampIndicators = useCallback(() => {
    ...
    const timestamps = modal.querySelectorAll('[data-ai-seo-timestamp]');
    timestamps.forEach((element) => {
      const raw = element.getAttribute('data-ai-seo-timestamp') || '';
  ...
  const handleSubmit = useCallback(async (data, action, submitFn) => {
    ...
    if (!hasErrors && isRegenerate) {
      const hadGeneratedSeo = formSchema?.meta?.hadGeneratedSeo;
      const noticeText = hadGeneratedSeo ? regenerateNoticeText : generateNoticeText;
...
export default connect(null, mapDispatchToProps)(AiSeoModal);
