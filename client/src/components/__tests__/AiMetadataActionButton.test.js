...
test('renders a share-style secondary toolbar button with metadata labelling', () => {
  ...
  expect(button.className).toContain('ai-metadata__action');
  expect(button.className).toContain('ai-metadata-toolbar__button');
  expect(button.className).toContain('btn-secondary');
  expect(button.getAttribute('data-fqcn')).toBe('App\\Page');
  expect(button.getAttribute('data-record-id')).toBe('7');
  expect(button.getAttribute('title')).toBe('Edit metadata');
