...
/**
 * Tracks per-session AI metadata regenerate requests for one member and record pair.
 */
class AiMetadataRegenerateRateLimiter
{
    private const SESSION_KEY = 'SilverstripeLtd.AiMetadata.RegenerateRateLimiter';
    ...
    {
        return max(1, (int)Config::inst()->get(AiMetadataRegenerateRateLimiter::class, 'max_requests'));
    }

    private function getWindowSeconds(): int
    {
        return max(1, (int)Config::inst()->get(AiMetadataRegenerateRateLimiter::class, 'window_seconds'));
    }
