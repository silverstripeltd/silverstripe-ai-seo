...
namespace SilverstripeLtd\AiSeo\Reports;
...
/**
 * Report on AI SEO status across SiteTree records.
 */
class AiSeoReport extends Report
{
    ...
    {
        return 'Generated AI SEO status';
    }
    ...
    {
        $statusFilter = $params['Status'] ?? AiSeoReport::STATUS_NEEDS_ATTENTION;
        $records = [];
        ...
        usort($records, function (ArrayData $a, ArrayData $b): int {
            $priority = [
                AiSeoReport::STATUS_MISSING => 1,
                AiSeoReport::STATUS_STALE => 2,
                AiSeoReport::STATUS_UNREVIEWED => 3,
                AiSeoReport::STATUS_OK => 4,
            ];
    ...
    private function determineStatus(
        ...
        AiSeoStateService $stateService
    ): string {
        if (!$metadata || !$metadata->exists() || !$metadata->GeneratedAt) {
            return AiSeoReport::STATUS_MISSING;
        }

        if ($metadata->ContentHash) {
            $state = $stateService->getState($page, $metadata);
            if ($state['stale']) {
                return AiSeoReport::STATUS_STALE;
            }
        }

        if (!$metadata->isReviewed()) {
            return AiSeoReport::STATUS_UNREVIEWED;
        }
        return AiSeoReport::STATUS_OK;
    }

    /**
     * Determine the live status of metadata.
     */
    private function determineLiveStatus(?GeneratedSeo $metadata): string
    {
        if (!$metadata || !$metadata->exists()) {
            return AiSeoReport::LIVE_NOT_PUBLISHED;
        }
        ...
        if ($metadata->isModifiedOnDraft()) {
            return AiSeoReport::LIVE_OUTDATED;
        }
        return AiSeoReport::LIVE_PUBLISHED;
    }
    ...
    private function matchesFilter(string $status, string $filter, string $liveStatus): bool
    {
        ...
        if ($filter === AiSeoReport::STATUS_ALL) {
            return true;
        }

        if ($filter === AiSeoReport::STATUS_NEEDS_ATTENTION) {
            return in_array(
                $status,
                [
                    AiSeoReport::STATUS_MISSING,
                    AiSeoReport::STATUS_STALE,
                    AiSeoReport::STATUS_UNREVIEWED,
                ],
                true
            )
                || ($status === AiSeoReport::STATUS_OK && $liveStatus === AiSeoReport::LIVE_NOT_PUBLISHED);
        }

        if ($filter === AiSeoReport::STATUS_NOT_PUBLISHED) {
            return $liveStatus === AiSeoReport::LIVE_NOT_PUBLISHED;
        }

        if ($filter === AiSeoReport::STATUS_OK) {
            return $status === $filter && $liveStatus !== AiSeoReport::LIVE_NOT_PUBLISHED;
        }
    ...
    {
        return [
            AiSeoReport::STATUS_NEEDS_ATTENTION => 'Needs attention',
            AiSeoReport::STATUS_MISSING => 'Missing',
            AiSeoReport::STATUS_STALE => 'Stale',
            AiSeoReport::STATUS_UNREVIEWED => 'Unreviewed',
            AiSeoReport::STATUS_OK => 'OK',
            AiSeoReport::STATUS_NOT_PUBLISHED => 'Not published',
            AiSeoReport::STATUS_ALL => 'All',
        ];
    }
    ...
    {
        $labels = [
            AiSeoReport::LIVE_PUBLISHED => 'Published',
            AiSeoReport::LIVE_NOT_PUBLISHED => 'Not published',
            AiSeoReport::LIVE_OUTDATED => 'Outdated',
        ];
